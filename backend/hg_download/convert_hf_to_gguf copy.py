...
def convert_model(model_path:Path=Path("/"), outfile:Path=Path("model.gguf"), outtype:str="q8_0") -> None:
    args = parse_args()
    ...
    if args.awq_path:
        ...
        tmp_model_path = model_path / "weighted_model"
        ...
        if tmp_model_path.is_dir():
            ...
            add_scale_weights(str(model_path), str(args.awq_path), str(tmp_model_path))
            logger.info(f"Saved weighted model at {tmp_model_path}.")

    if not dir_model.is_dir():
        logger.error(f'Error: {model_path} is not a directory')
        sys.exit(1)
    ...
    with torch.inference_mode():
        ...
        model_instance = model_class(dir_model, ftype_map[outtype], fname_out, args.bigendian, args.use_temp_file, args.no_lazy, args.model_name)
